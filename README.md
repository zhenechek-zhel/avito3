...
- **id** - уникальный идентификационный номер чата может быть несколько чатов (чат покапатель-продавец, чат тех. поддержка);
- **members** - список Пользователей* которые участвуют в чате. Разные пользователи могут писать в разные чаты (@ManyToMany);
- **messages** - список Сообщений* который содержит чат.Разные сообщения могут публиковаться в разных чатах. (@ManyToMany);
...
- **categories** - список Категорий* к которым относится данный товар(ManyToMany);
...
- **shop** - описывает Магазин* в котором продается товар(ManyToOne);
...
- **location** - поле хранящее Страну* расположения магазина(ManyToOne);
- **items** - поле хранящее список Товаров* продающихся в магазине(OneToMany);
- **reviews** - поле хранящее список Отзывов* на магазин(OneToMany);
- **logo** - поле хранящее Изображение* магазина(логотип)(OneToOne);
- **rating** - поле показывающее рейтинг(в числовом выражении) магазина;
- **user** - поле хранящее Пользователя* которому принадлежит магазин(ManyToOne);
- **discounts** - поле хранящее коллекцию Скидок* выдаваемую данным магазином(OneToMany);
- **isModerated** - поле отражающее статус промодерировано/не промодерировано;
- **isModerateAccept** -поле отражающее статус после модерации, прошел модерацию/не прошел модерацию;

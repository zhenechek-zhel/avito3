...
- **id** - уникальный идентификационный номер купона;
- **start** - дата начала действия купона;
- **end** - дата окончания действия купона;
- **shop** - поле хранящее Магазин* который выдал купон;

```
Сущность предоставляющая поля для определения срока действия скидочного купона. 
Имеет связи с Магазином*, в котором был выдан.
```
### Discount:
